...
#self.cursor.execute( """CREATE TABLE IF NOT EXISTS `%s`(%s PRIMARY KEY( `%s` ))Engine = MyISAM""" %(name, all_columns, primary_key))      //takes in a dictionary for columns names: datatypes
    ...
    #returns the max value from a given column from a given table
    def select_max(self, column, table):
        self.cursor.execute("SELECT MAX(%s) FROM %s" %(column, table))
        result = self.cursor.fetchall()
        return(result)#retuned as a ditcionary
        
    #queries the database to get the most recent Neural network. if the status is 0, then it should be sent to unity for testing
    #if 1, then it is currently being simulated , if 2, then it has been simulated. 
    #in the case of 1 or 2, a new Neural network should be added to the database and queued for testing
    def get_status(self):
        self.cursor.execute("SELECT STATUS, MAX(CONTROLLER_NUMBER) FROM CONTROLLER_TABLE GROUP BY STATUS")
        result = self.cursor.fetchall()
        return(result)
    
    #collects all data from the spesified table
    def select_whole_table(self, table):
        self.cursor.execute("SELECT * FROM %s" %(table))
        result = self.cursor.fetchall()
        return (result)
    
    #gets the status of the controller table to deturmine if a new neural network should be created.
    def update_status(self, value, NN_number):
        self.cursor.execute("UPDATE CONTROLLER_TABLE SET STATUS = %s WHERE CONTROLLER_NUMBER = %s" %(value, NN_number))
#_________________________________________________^^working   


#____________________________________________________Not Tested Yet
...
''' 
     #table as string, s_columns(select columns) as list, max_column as string, 
    def group_by_max(self, table, s_columns, max_column, g_column):
        all_columns = itterate_list(s_columns)      
        self.cursor.execute("SELECT %s, MAX(%s) AS %s FROM %s GROUP BY %s" %(all_columns, max_column, max_column, table, g_column))
        print("SELECT %s, MAX(%s) AS %s FROM %s GROUP BY %s" %(all_columns, max_column, max_column, table, g_column))
